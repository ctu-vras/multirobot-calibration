...
%            %old - for k=1:size(cop,2)
%                %old - if distance of point to any taxel in COP is lower than
%                %old - 'maxDist' => point belongs to this COP
%                % if point is in 'maxDist' range to first point in this COP
%                 if norm(point-cop{1},2)<maxDist
%                     %Assing point to COP
%                     cops{j}{end+1}=point;
%                     %Set inRange to 1
%                     inRange=1;
...
%     end
    ...
    [~, ~, copsIdx] = MeanShiftCluster(points(:,1:3)',maxDist);
%     cops_ = cops;
    cops = {};
    off = 0;
    for idx=1:length(copsIdx)
        s = size(copsIdx{idx},2);
        if s==0
            off = off +1;
        end
        for l=1:size(copsIdx{idx},2)
           if l==1
              cops{end+1} = {points(copsIdx{idx}(l),1:3)};
           else
              cops{idx-off}{end+1}=points(copsIdx{idx}(l),1:3); 
           end
        end
%         cops{end+1}=temp;
    end
    centers_ = zeros(length(cops),3);
    for idx=1:length(cops)
        temp = reshape(cell2mat(cops{idx}), 3 , [])';
        centers_(idx,:) = [mean(temp(:,1)),mean(temp(:,2)),mean(temp(:,3))];
    end
    finalCops = centers_;
end