...
for pert_level = 1:(1+length(optim.pert(optim.pert==1)))
    for rep = 1:1 %optim.repetitions    
        ...
        tr_datasets = getDatasetPart(datasets, training_set_indexes{1});
        for field = 1:length(fnames)
           dh.(fnames{field}) = start_dh.(fnames{field})(:,:, 1, pert_level);
        end
        vals = errors_fcn(start_pars(:,1,pert_level), dh, rob, rob.structure.WL, tr_datasets, optim);
        hist(vals)
